...
Google-style docstrings required on all modules, classes, and public functions. Include Args, Returns, Raises, Examples sections. Update docstrings when modifying code behavior.
...
JWT verification core for OIDC/JWKS. Public API: `AuthConfig`, `AuthError`, `JWTVerifier`.
...
- **verifier.py** - `JWTVerifier.verify_access_token()`: rejects forbidden headers (`jku`, `x5u`, `crit`), validates `alg`/`kid`, fetches JWKS key, verifies signature and claims, enforces scopes/permissions.

Security: fail-closed, never derives JWKS URL from token headers, explicit algorithm allowlist.
